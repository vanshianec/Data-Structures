...
public class PTest16 {
    ...
    @Test
    public void getAllInvoiceInPeriod_10000_elements() {

        List<Invoice> invoices = this.generator.generateInvoices(10000);

        invoices.forEach(i -> this.agency.create(i));
        Optional<Map.Entry<LocalDate, Long>> mostOccurrencesIssueDate = this.generator.getMostOccurrences(invoices, Invoice::getIssueDate);

        Optional<Map.Entry<LocalDate, Long>> mostOccurrencesDueDate = this.generator.getMostOccurrences(invoices, Invoice::getDueDate);

        long start = System.currentTimeMillis();
        this.agency.getAllInvoiceInPeriod(mostOccurrencesIssueDate.get().getKey(), mostOccurrencesDueDate.get().getKey());
...
//    throw new IllegalArgumentException("Time: " + elapsedTime);
        ...
        Assert.assertTrue(elapsedTime <= 80);
