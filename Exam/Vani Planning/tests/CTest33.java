...
public class CTest33 {
    ...
    @Test
    public void getAllByCompany_should_return_empty_collection() {
        ...
        Invoice[] actualCount = StreamSupport
                .stream(this.agency.getAllByCompany("SoftUni").spliterator(), false)
                .toArray(Invoice[]::new);

        Assert.assertEquals("Incorrect order", inv3.getNumber(), actualCount[0].getNumber());

        Assert.assertEquals("Incorrect order", inv2.getNumber(), actualCount[1].getNumber());
    }
}