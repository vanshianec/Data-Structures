...
public class PTest25 {
    ...
    @Test
    public void getAllFromDepartment_10000_elements() {

        List<Invoice> invoices = this.generator.generateInvoices(10000);

        invoices.forEach(i -> this.agency.create(i));

        Optional<Map.Entry<Department, Long>> mostOccur = this.generator.getMostOccurrences(invoices,Invoice::getDepartment);

        Department mostOccurDepartment = mostOccur.get().getKey();
