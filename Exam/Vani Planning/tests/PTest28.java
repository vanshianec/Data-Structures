...
public class PTest28 {
    ...
    @Test
    public void getAllByCompany_10000_elements() {

        List<Invoice> invoices = this.generator.generateInvoices(10000);

        invoices.forEach(i -> this.agency.create(i));

        Optional<Map.Entry<String, Long>> mostOccur = this.generator.getMostOccurrences(invoices,Invoice::getCompanyName);

        String mostOccurCompanyName = mostOccur.get().getKey();

        long start = System.currentTimeMillis();
        this.agency.getAllByCompany(mostOccurCompanyName);
...
//    throw new IllegalArgumentException("Time: " + elapsedTime);
        ...
        Assert.assertTrue(elapsedTime <=10);
